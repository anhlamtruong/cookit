...
const StoreSwitcher = ({ className, items = [] }: StoreSwitcherProps) => {
  ...
  const onStoreSelect = (store: { value: string; label: string }) => {
    setOpen(false);
    router.push(`/store_admin/${store.value}`);
  };
  ...
  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        ...
        >
          <ChevronsUpDown className=" mr-auto h-4 w-4 shrink-0 opacity-50"></ChevronsUpDown>
          {currentStore?.label ?? ""}
          <StoreIcon className=" ml-4 h-4 w-4"></StoreIcon>
        </Button>
      </PopoverTrigger>
